# crypto_hunter_web/services/import_service.py
"""
...
class ImportService:
    ...
    def import_from_csv(csv_path, user_id):
        ...
        try:
            ...
            # Local import of AuthService to avoid circular dependency
    ...
    def _process_csv_row(row, bulk_import, user_id):
        ...
        data = {f: ImportService._get_field_value(row, names)
                for f, names in field_mappings.items()}
        ...
        if not data['sha256_hash']:
            if data['filepath'] and os.path.exists(data['filepath']):
                ...
                fake = f"{data['filename']}{data['filepath']}{data['file_size']}{user_id}".encode()
                data['sha256_hash'] = hashlib.sha256(fake).hexdigest()
        ...
        try:
            ...
            file_size = os.path.getsize(data['filepath']) if data['filepath'] and os.path.exists(data['filepath']) else 0
        ...
        if data['filepath'] and os.path.exists(data['filepath']) and file_size < 50 * 1024 * 1024:
            try:
    ...
    def _get_file_type_color(file_type):
        ...
        ft = file_type.lower() if file_type else 'application'
    ...
    def create_sample_csv(output_path='sample_import.csv', num_rows=100):
        ...
        for i in range(num_rows):
            ...
            file_types = [
                ('image.jpg','image/jpeg'), ('audio.wav','audio/wav'),
                ('document.pdf','application/pdf'), ('archive.zip','application/zip'),
                ('data.bin','application/octet-stream'), ('text.txt','text/plain')
            ]
            filename, ftype = random.choice(file_types)
            sample_data.append({
                ...
                'description': f"Sample file {i} for testing",
                'is_root': 'true' if i%20==0 else 'false',
                'extraction_method': random.choice(['manual','zsteg','steghide','binwalk'])
            })
    ...
    def import_from_directory_scan(directory_path, user_id, extensions=None, recursive=True):
        ...
        try:
            ...
            file_count = imported = 0
            walker = os.walk if recursive else lambda d: [(d,[],os.listdir(d))]

            for root, dirs, files in walker(directory_path):
                for file in files:
                    ...
                    try:
                        sha = ImportService._calculate_sha256(file_path)
                        if not sha:
                            continue
                        existing = AnalysisFile.query.filter_by(sha256_hash=sha).first()
                        if existing:
                            bulk_import.duplicates_found += 1
                            continue
                        ...
                        ftype = ftype or 'application/octet-stream'
                        ...
                        af = AnalysisFile(
                            ...
                            is_root_file=False,
                            status='pending',
                            node_color=ImportService._get_file_type_color(ftype),
                        ...
                        if size < 20*1024*1024:
                            try:
                                FileAnalyzer.analyze_file_content(file_path, af.id)
                            except:
                                pass

                        if imported % 100 == 0:
                            db.session.commit()
                            logger.info(f"Directory scan: processed {imported} files...")

                    except Exception as e:
                        bulk_import.errors_count += 1
                        ImportService._log_error(bulk_import, f"Error processing {file_path}: {e}")
            ...
            bulk_import.successful_imports = imported
            bulk_import.status = 'completed'
            bulk_import.completed_at = datetime.utcnow()
            db.session.commit()

            logger.info(f"Directory scan completed: {imported} files imported from {directory_path}")

            # Local import of AuthService
            from crypto_hunter_web.services.auth_service import AuthService
            AuthService.log_action(
                'directory_scan_completed',
                f'Scanned {directory_path}, imported {imported} files',
                user_id
            )
